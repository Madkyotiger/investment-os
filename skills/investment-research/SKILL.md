...
7. **China-source endpoint fragility.** AKShare endpoints do not fail uniformly: Eastmoney historical endpoints such as `fund_etf_hist_em` / `stock_zh_a_hist` may fail behind proxy or source blocking, while `fund_etf_spot_em` and `stock_zh_index_daily` can still work. For the first China-market tracer bullet, prefer a live spot/index probe plus explicit Evidence Ledger gaps over pretending the full historical layer is ready. Tushare must be credential-gated: if `TUSHARE_TOKEN` is absent, write `token_missing` as evidence instead of silently omitting it.
...
9. **China dual-source reconciliation.** Once AKShare and Tushare are both wired, add an explicit reconciliation gate row rather than leaving comparison implicit. If `TUSHARE_TOKEN` is missing, write `blocked_by_tushare_token_missing` / `missing` as reconciliation evidence; if both sources exist, compare at least trade date and close field before trusting downstream summaries.
...
15. **External note renderers need their own QA gate.** Once a pilot ledger can produce an internal memo, add a separate external renderer that reads evidence rows and writes a forwardable research note. Pair it with a deterministic quality scan for local paths, cache paths, run commands, implementation labels, and decision/execution language; keep credential gaps such as missing Tushare tokens visible as source gaps, not hidden errors or conclusions.
