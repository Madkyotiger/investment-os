...
7. **Ship machine artifacts with the note.** Preserve inputs, scenarios, stress rows, threshold rows, valuation reproduction, source supplement, summary JSON, deterministic validator, and a compact receipt. Complete final edits before hashing; verify receipt hashes before writing back to the owning project/state surface.
